...
OpenAI's Harness Engineering represents a paradigm shift: **the primary job of engineering becomes enabling agents to do useful work**. This means working depth-first — breaking down larger goals into smaller building blocks (design, code, review, test), prompting agents to construct those blocks, and using them to unlock more complex tasks.

OpenAI built an internal product over five months with **zero manually-written code** — approximately **one million lines across 1,500 pull requests**, all generated by Codex agents.

### Key Harness Engineering Principles

1. **Repository-Centered Context** — Favor dependencies and abstractions that can be fully internalized and reasoned about in-repo
2. **Golden Principles** — Encode opinionated, mechanical rules directly into the repository
3. **Recurring Cleanup Process** — Background Codex tasks that scan for deviations, update quality grades, and open targeted refactoring PRs
4. **Technical Debt Management** — Functions like garbage collection, keeping the codebase legible and consistent
5. **Depth-First Decomposition** — Break larger goals into atomic building blocks

---

## Current State Analysis

### ✅ What Engram Already Does Well
...
if [ $? -ne 0 ]; then
  echo "❌ Golden principles violations detected. Fix before committing."
  exit 1
fi
```
...
```
agents/cleanup/
├── drift-detector.ts      # Scans for violations of golden principles
├── refactor-suggester.ts  # Generates PR descriptions for fixes
├── quality-updater.ts     # Updates QUALITY-REPORT.md
└── scheduled-cleanup.ts   # Cron job orchestrator
```
...
export async function detectAndFixDrift() {
  ...
  for (const violation of violations.filter(v => v.severity === 'critical')) {
    ...
    // Open PR
    await openGitHubPR({
      title: `🤖 Fix ${violation.principle}: ${violation.message}`,
      body: `
## Automated Cleanup
...
├── new-mcp-tool.md               # Template for adding a new MCP tool
├── convex-table.md               # Template for adding a Convex table
├── cron-job.md                   # Template for adding a cron job
├── lifecycle-hook.md             # Template for adding a Claude Code hook
└── feature-implementation.md     # Generic feature template
```
...
- Files: 42
- Average grade: A-
- Trend: ↑ (improved from B+ last week)
...
## Recent Improvements
1. Fixed GP-004 violations in 12 files (console.log → console.error)
2. Refactored `memory_get_context` into 7 atomic primitives
3. Updated all Convex paths to use type-safe constants

## Recommended Actions
1. 🔴 Urgent: Refactor `legacy-tool.ts` (Grade C)
2. 🟡 This week: Improve CLI module to Grade A (add missing tests)
3. 🟢 Next sprint: Add integration tests for new primitives
```
...
export const updateGoldenPrinciples = internalAction({
  handler: async (ctx) => {
    ...
    if (analysis.newPrinciples.length > 0) {
      ...
      await openGitHubPR({
        title: '🧠 Update Golden Principles (Agent Learning)',
        body: `
